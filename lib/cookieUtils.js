...
// TODO: Refactor to use localstorage instead of cookiePermission
let cookieDb = {
  _db: {},
  init: function(){
    if(!storage.cookieDb){
    ...
    this._db = storage.cookieDb;
  },
  setAccess: function(location, setting){
    let origin =  location.host;
    this._db[origin] = setting;
  },
  canAccess: function(location /*, context*/){
    let origin =  location.host;
    return this._db[origin];
  },
  remove: function(location) {
    let origin =  location.host;
    delete this._db[origin];
  },
  hasEntry: function(location){
    let origin =  location.host;
    return this._db.hasOwnProperty(origin);
  }
};
cookieDb.init();
...
let getCookiesFromChannel = exports.getCookiesFromChannel = function(channel) {
  try {
    let cookieString = cookieService.getCookieString(channel.URI, channel) ||
      cookieService.getCookieStringFromHttp(channel.URI, channel.URI, channel);
...
let clobberCookie = exports.clobberCookie = function(host) {
  // TODO: do nothing if the user already has more restrictive permissions
  // eg: no cookies ever, no 3rd party cookies ever, || location _DENY
  ...
  // is third-party.
  if (host === "localhost") {
    cookieDb.setAccess(location, _DENY);
  } else {
    cookieDb.setAccess(location, _DENY_THIRD_PARTY);
  }
  storage.changedCookies[host] = true;
};

function _unclobberCookies(host){
  let location = ABPUtils.makeURI("http://"+host);
  cookieDb.remove(location);
...
/**
 * Restore cookie setting for a domain and all subdomains to default.
 * @param {String} host
 * @return {void}
 */
let resetCookie = exports.resetCookie = function(host) {
  _unclobberCookies(host)
  for(var subdomain in cookieDb._db){
    if(utils.isSubdomain(subdomain, host)){
      _unclobberCookies(subdomain);
    }
  }
}
...
let allowCookie = exports.allowCookie = function(host) {
  let location = ABPUtils.makeURI("http://"+host);
  cookieDb.setAccess(location, _ALLOW);
  storage.changedCookies[host] = true;
};
...
let canAccessCookies = exports.canAccessCookies = function(location, channel) {
  ...
  let access;
  if(cookieDb.hasEntry(location)){
    access = cookieDb.canAccess(location, channel);
  } else {
    access = cookiePermission.canAccess(location, channel);
  }
  switch (access) {
    case _ALLOW:
      return true;
