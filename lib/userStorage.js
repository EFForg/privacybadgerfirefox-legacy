  ...
  Request({
    ...
    onComplete: function(response) {
      if (response.status == 200) {
        ...
        console.error("Error: could not get", backupDomainExceptionURL);
      }
    }
  }).get();
};
 ...
 * @param {String} host The tracker to unblock
 ...
 */
exports.addToDomainExceptions = function(host, trackingHost, context) {
...
exports.removeFromDomainExceptions = function(url, trackingHost, context) {
  let host = ABPUtils.makeURI(url).host; doDependingOnIsPrivate("domainExceptions",
                         function(store) {
...
exports.isDomainException = function(url, trackingHost, context) {
  ...
  if (!location || !location.host || !trackingHost) { 
    return false; 
  }
