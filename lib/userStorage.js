...
function syncExceptions(json) {
  storage.domainExceptionSites = _parseExceptionsJSON(json) ||
                                 storage.domainExceptionSites;
...
function _parseExceptionsJSON(json) {
  let finalJSON = {};
  for (let schemelessURL in json) {
    finalJSON["http://" + schemelessURL + "/"] = json[schemelessURL];
    finalJSON["https://" + schemelessURL + "/"] = json[schemelessURL];
  }
  return finalJSON;
}
...
exports.sync = function()
{
  ...
  Request({
    url: backupDomainExceptionURL,
    onComplete: function(response) {
      if (response.status == 200) {
        syncExceptions(response.json);
      } else {
        console.log("Error: could not get", backupDomainExceptionURL);
      }
    }
  }).get();
};
...
exports.addToDomainExceptions = function(url, trackingHost, context) {
  let host = ABPUtils.makeURI(url).host;
  doDependingOnIsPrivate("domainExceptions",
                         function(store) {
...
exports.isDomainException = function(url, trackingHost, context) {
  let location;
  try {
    location = ABPUtils.makeURI(url);
  } catch(e) {
    return false;
  }
  if (!location || !location.host || !trackingHost) { return false; }
  // Exception applies if the tracker domain or one of its parents is in the
  // set of exceptions for URL
  return doDependingOnIsPrivate("domainExceptions",
                                function(store) {
                                  ...
                                  return utils.checkEachParentDomainString(
                                    trackingLocation,
                                    function(host) {
                                      return (host in store[location.host]);
                                    }
                                  );
                                },
                                context);
