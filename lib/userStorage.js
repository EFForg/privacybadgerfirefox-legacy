...
/**
 * Return an array of disabled domains,
 * used for excluding content scripts.
 * If privacy badger is disabled returns ["*"]
 */
exports.disabledSitesArray = function(){
  console.log("GET DISABLED SITES");
  if(utils.isHeuristicEnabled()){
    let domains = Object.keys(storage.disabledSites);
    if(!domains || domains.length === 0){
      // One does not simply return an empty array
      return "*.fake.eff.org";
    }
    domains = domains.map(function(dom){ return dom + "/*"; });
    return domains;
  } else {
    return ["*"];
  }
...
exports.setSeenComic = function(setting){
  storage['seenComic'] = setting;
}

exports.getSeenComic = function(setting){
  return storage['seenComic'] || false;
}
