...
let doDependingOnIsPrivate = exports.doDependingOnIsPrivate =
  ...
  let isPrivate = isPrivate(context);
  if (isPrivate) {
 ...
 * @param {string} host
 * @param {object} context true for private BrowserWindows, tabs, and workers
 */

let add = exports.add = function(color, host, context)
{
  let storeName;
  switch(color) {
    case "red":
      storeName = "userRed";
      remove("yellow", host, context);
      remove("green", host, context);
      break;
    case "yellow":
      storeName = "userYellow";
      remove("red", host, context);
      remove("green", host, context);
  ...
  doDependingOnIsPrivate(storeName,
                         function (store) { store[host] = true; },
                         context);
};
let remove = exports.remove = function(color, host, context)
{
  let storeName;
  switch(color) {
    case "red":
...
exports.clearOrigin = function(host, context) {
  exports.remove("red", host, context);
  exports.remove("yellow", host, context);
  exports.remove("green", host, context);
};
...
function _clearUserCookieSettings() {
  ...
  for (let host in storage.userGreenPrivate) {
    if (storage.userGreen.hasOwnProperty(host)) {
      cookieUtils.resetCookie(host);
    }
  }
  for (let host in storage.userYellowPrivate) {
    if (storage.userYellow.hasOwnProperty(host)) {
      cookieUtils.resetCookie(host);
    }
  }
