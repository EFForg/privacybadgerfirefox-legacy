...
let changedCookies = "changedCookies";
...
exports.init = function()
{
  ...
  // Keeps track of hosts for which we've overriden default cookie settings
  if (!storage.changedCookies) {
    storage.changedCookies = new Set();
  }
};
...
exports.empty = function()
{
  ...
  userStored.forEach(
    function(store)
    {
      // Don't clear preloads and disabledSites
...
exports.isDisabledSite = function(url) {
  try {
    var host = ABPUtils.makeURI(url).host;
  } catch(e) {
    // Sometimes URL is null or doesn't have a host
    return false;
  }
  return (host ? (host in storage.disabledSites) : false);
};
