  ...
  if (isSocialWidgetReplacementEnabled()) {
    ...
    tabs.on('ready', function(tab) {  
      ...
      worker.port.on("socialWidgetContentScriptReady", function(aWin) {
        let socialWidgetContentScriptData = socialWidgetHandler.getSocialWidgetContentScriptData(tab);
	socialWidgetContentScriptData["socialWidgetReplacementEnabled"] = isSocialWidgetReplacementEnabled();
        worker.port.emit("socialWidgetContentScriptReady_Response", socialWidgetContentScriptData);
      });
    ...
    // Clear temporary unblocks on tab close
    // (Note that browser quit will also cause tabs to close)
    tabs.on('close', function(tab) {
      socialWidgetHandler.clearTemporaryUnblocksByTab(tab);
    });
  }
