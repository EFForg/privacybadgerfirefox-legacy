...
function enable() {
  ...
  // DEBUG
  /*
   * For some reason, the add-on SDK's wrapper around nsIObserverService
   * wasn't receiving any cookie-changed events. ???
   *
  events.on("cookie-changed", function onCookieChanged(event) {
    // Ignore events that don't have a subject (which is legitimate, see
    // http://dxr.mozilla.org/mozilla-central/source/netwerk/cookie/nsICookieService.idl#31)
    if (!event.subject) {
      return;
    }
    console.log("cookie-changed :: " + event.data + " cookie for " + event.subject.host);
  }, false);
  */
  Cu.import("resource://gre/modules/Services.jsm");
  Services.obs.addObserver(function cookieObserver(aSubject, aTopic, aData) {
    if (!aSubject) {
      return;
    }
    let cookie = aSubject.QueryInterface(Ci.nsICookie);
    console.log("cookie-changed :: " + aData + " cookie for " + cookie.host);
...
function main(options) {
  ...
  try {
    ...
    // pbUI.onStartup may fire before gBrowser is available.
    // XXX: Listen to sessionstore-windows-restored here instead?
