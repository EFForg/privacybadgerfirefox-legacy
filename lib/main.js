...
function webexStorageFile() {
  var fHandle = Cc['@mozilla.org/file/directory_service;1']
    .getService(Ci.nsIProperties)
      .get('ProfD', Ci.nsIFile); //get the profile direcotry

  // Get to the new web extensions storage path
  fHandle.append('browser-extension-data');
  fHandle.append(self.id);
  fHandle.append('storage.js');
  if(!fHandle.exists()) { fHandle.create(0, 0o600); }

  return fHandle;
}

function migrateData(store){
  let outfile = webexStorageFile().path;
  let TextWriter = fileIO.open(outfile, "w");
  if (!TextWriter.closed) {
    TextWriter.writeAsync(store, function(){
      console.log("COPYING PRIVACY BADGER DATA TO WEB EXTENSIONS PATH");
    });
  } else {
   console.error("privacy badger: ERROR: migration file was closed before we \
    could write to it");
  }
}
...
function updateStorageCache() {
  var stored = JSON.stringify(storage);
  if(storCache != stored) {
    storCache = stored;
    migrateData(storCache);
  }
}

/**
 * Update storage cache every 24 hours. Note: I am aware that
 * this is not the most optimal possible solution. This is less IO than
 * writing every time our accounting structure updates (which happens on every
 * page load. The best solution would be to do this in onUnload() but our tests
 * showed that this doesn't actually work. Given all that we think that this is
 * the solution which has the least performance impact while still retaining
 * most user data. 
 */
setInterval(updateStorageCache, 1000 * 60 * 60 * 24);
updateStorageCache();
...
function enable() {
  ...
  if (isSocialWidgetReplacementEnabled()) {
    ...
    tabs.on('ready', function(tab) {
      let worker = tab.attach({
        contentScriptFile: self.data.url("socialwidgets.js")
      });
  ...
  pageMod.PageMod({
    ...
    contentScriptFile: [self.data.url("supercookie.js"), self.data.url("fingerprinting.js")],
...
function onContentDocumentGlobalCreated(event){
  ...
  if(host != topHost && cookieblock ){
    ...
    subScriptLoader.loadSubScript(self.data.url("clobbercookie.js"), aWin.document);
  }
}
