...
function onModifyRequest(event) {
  ...
  // If disabled on the current page, inject cookies into the channel that
...
let ignoreRequest = function(channel) {
  ...
  if (!(ContentPolicy.Policy.isBlockableRequest(channel.URI, null, channel))) {
    console.log("non-blockable request", channel.URI.spec);
    return true;
  }
...
function enable() {
  ...
  pbUI.onWindowOpen();
  windows.on("open", function(window) { pbUI.onWindowOpen(); });
  ...
  tabs.on('ready', function(tab) {
    ...
    worker.port.on("socialWidgetContentScriptReady", function() {
        console.log("CONTENT SCRIPT CALL: socialWidgetContentScriptReady");
        let socialWidgetBlockList = getSocialWidgetBlockList(tab.id);
        worker.port.emit("socialWidgetContentScriptReady_Response", socialWidgetBlockList);
    });

    worker.port.on("unblockSocialWidget", function(buttonUrls) {
        console.log("CONTENT SCRIPT CALL: unblockSocialWidget");
        let socialWidgetUrls = request.buttonUrls;
        unblockSocialWidgetOnTab(tab.id, socialWidgetUrls);
...
// TODO: MOVE THE BELOW TO OWN CLASS
