...
function recordSupercookie(tab, report){
  if(!report.scriptUrl){return false;}
...
function testCanvasFingerprint(script_origin, msg){
  ...
  if (!settingsMap.canvasTracking.hasOwnProperty(script_origin)) {
    settingsMap.canvasTracking[script_origin] = {
      canvas: {
  ...
  var scriptData = settingsMap.canvasTracking[script_origin];
