...
function recordFingerprinting(tab, report){
  console.log('*****REPORT*****\n',report,tab);
  var origin = utils.getBaseDomain(utils.makeURI(report.scriptUrl));
  var parentURI = utils.makeURI(tab.url);
  var parentFQDN = parentURI.host;
  var parentOrigin = utils.getBaseDomain(parentURI);
  if(report.extra && report.extra.canvas){
    if (utils.isThirdPartyURI(report.scriptUrl, tab.url)){
      heuristicBlocker.recordPrevalence(origin, parentOrigin, parentFQDN, tab.window);
    }
  }
}
