...
function onModifyRequest(event) {
  ...
  if (isHeuristicEnabled()) {
    heuristicBlocker.updateHeuristicsForChannel(channel, aWin, "request");
  }
...
function onExamineResponse(event) {
  ...
  if (isHeuristicEnabled()) {
    heuristicBlocker.updateHeuristicsForChannel(channel, aWin, "response");
  }
}
...
// Called on application shutdown to respect user pref to clear all cookies
// on shutdown.
function deletePbCookies(event) {
  // values() only supported in FF24 and above. :(
  let hostsIter = storage.changedCookies.values();
  while (true) {
    let host = hostsIter.next();
    if (host.value) {
      cookieUtils.clearCookiesForHost(host.value);
    } else {
      break;
    }
  }
...
exports.deletePbCookies = deletePbCookies;
