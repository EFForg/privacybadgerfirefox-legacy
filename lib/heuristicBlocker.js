...
let blockOrigin = function(origin) {
  // If an origin is considered block-worthy, check to see if they publish an
  // approved privacy policy before blocking them
  checkPrivacyPolicy(origin, function(success) {
    if (success) {
      console.debug("adding" + origin + "to user whitelist due to privacy policy");
      if (storage.blockedOrigins.hasOwnProperty(origin)) {
	unblockOrigin(origin);
      }
      // TODO: should we add this to its own whitelist? At least this way, the
      // user can still override this decision if they want to.
      storage.userGreen[origin] = true;
    } else {
