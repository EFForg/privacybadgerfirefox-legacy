...
let hasTracking = function(channel, channelInfo) {
  ...
  if (!(origin in gSetCookieOriginFrequency)) {
    gSetCookieOriginFrequency[origin] = { };
  }
  gSetCookieOriginFrequency[origin][parentOrigin] = true;
...
let updateHeuristics = function(channel) {
  ...
  if (!(origin in gHttpRequestOriginFrequency)) {
    gHttpRequestOriginFrequency[origin] = { };
  }
  // This 3rd party tracked this 1st party
  gHttpRequestOriginFrequency[origin][parentOrigin] = true;
  console.log(origin + " tracked " + parentOrigin);

  let httpRequestPrevalence =
    Object.keys(gHttpRequestOriginFrequency[origin]).length;
  if (httpRequestPrevalence >= gPrevalenceThreshold) {
    console.log("Adding " + origin + " to heuristic blocklist.");
    heuristicBlockOrigin(origin);
  }
