...
// TODO: persist to simple-storage
let gHttpRequestOriginFrequency = { };
let gSetCookieOriginFrequency = { };
...
let hasTracking = function(channel, channelInfo) {
  ...
  if (!(channelInfo.origin in gSetCookieOriginFrequency)) {
    gSetCookieOriginFrequency[channelInfo.origin] = { };
  }
  gSetCookieOriginFrequency[channelInfo.origin][channelInfo.parentOrigin] = true;
...
let heuristicBlockOrigin = function(origin) {
  gBlockedOrigins[origin] = true;
...
let updateHeuristics = function(channel) {
  ...
  if (channelInfo.party == "first") return false;
  ...
  if (!(channelInfo.origin in gHttpRequestOriginFrequency)) {
    gHttpRequestOriginFrequency[channelInfo.origin] = { };
  }
  // This 3rd party tracked this 1st party
  gHttpRequestOriginFrequency[channelInfo.origin][channelInfo.parentOrigin] = true;
  console.log(channelInfo.origin + " tracked " + channelInfo.parentOrigin);

  let httpRequestPrevalence =
    Object.keys(gHttpRequestOriginFrequency[channelInfo.origin]).length;
  if (httpRequestPrevalence >= gPrevalenceThreshold) {
    console.log("Adding " + channelInfo.origin + " to heuristic blocklist.");
    heuristicBlockOrigin(channelInfo.origin);
  }
