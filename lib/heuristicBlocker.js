...
let hasTracking = function(channel, channelInfo, cookies) {
  let tracking = hasCookieTracking(channel, channelInfo, cookies) ||
...
var hasSupercookieTracking = function(channel, channelInfo){
  return settingsMap.supercookies && 
    !!settingsMap.supercookies[channelInfo.origin];
};

var hasCanvasFingerprinting = function(channel, channelInfo){
  return settingsMap.canvasTracking && 
    settingsMap.canvasTracking[channelInfo.origin] &&
    !!settingsMap.canvasTracking[channelInfo.origin].fingerprinting;
};
...
let updateHeuristicsForChannel = function(channel, win, isResponse) {
  ...
  if (!hasTracking(channel, channelInfo, channelCookies)) { 
      emit(settingsMap, "update-settings", "notracking", win, channel.URI.host);
      return false; 
  }
