...
/**
 * Fires when window location changes or is closed.
...
exports.onWindowOpen = function(event) {
  ...
  PBListener(browser);
  browser.tabContainer.addEventListener("TabClose", function(event) {
    // get nsIDOMWindow for the tab closed
    console.log("Got tab close");
    let win = tabUtils.getTabContentWindow(event.target);
    onLocationChange(win);
  }, false);
};
