 ...
 * @param {Function} iterations Number of times function has been called so far. Internal use only.
 */
function repeatAtRandom(callee, min, max, callLimit, callback, iterations){
  if(callLimit){
    var iterations = iterations || 0;
    console.log('calling number', iterations, 'of', callLimit);
    if(iterations == callLimit){
  ...
  timers.setTimeout(function(){
    callee();
    repeatAtRandom(callee, min, max, callLimit, callback, iterations);
    return true;
  }, callTime);
}
