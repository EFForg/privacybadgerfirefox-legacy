...
exports.ContentPolicy = Class({
  ...
  shouldLoad: function(contentType, contentLocation, requestOrigin, node, mimeTypeGuess, extra)
  {
    ...
    if (contentLocation.host in storage.userRed) {
      emit(settingsMap, "update-settings", "userblock", window, contentLocation.host);
      return Ci.nsIContentPolicy.REJECT_REQUEST;
    }

    if (contentLocation.host in storage.blockedOrigins) {
      emit(settingsMap, "update-settings", "block", window, contentLocation.host);
      return Ci.nsIContentPolicy.REJECT_REQUEST;
    }
...
let Policy = exports.Policy =
{
  ...
  isWhitelisted: function(location, aWin) {
    ...
    if (location.host in storage.userGreen) {
      emit(settingsMap, "update-settings", "usernoaction", aWin, location.host);
      return "userGreen";
    }

    // Is it on the preloaded whitelist?
    if (location.host in storage.preloads) {
      emit(settingsMap, "update-settings", "noaction", aWin, location.host);
      return "preloads";
    }
  },
  ...
  {
    return ((location.host in storage.userRed) ||
            (location.host in storage.blockedOrigins));
  }
};
