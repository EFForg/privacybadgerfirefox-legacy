...
let Policy = exports.Policy =
{
  ...
  heuristicBlocksRequest: function(location, window)
  {
    ...
    // Is the request's hostname on the heuristic blocklist?
    let host = utils.getBaseDomain(location);
  ...
  heuristicCookieblocksRequest: function(location, window)
  {
    ...
    // Is the request's base domain on the heuristic blocklist AND host in the
    // preloaded whitelist?
    if (utils.getBaseDomain(location) in storage.preloads) {
      if (host in storage.blockedOrigins) {
