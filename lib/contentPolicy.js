...
exports.ContentPolicy = Class({
  ...
  shouldLoad: function(contentType, contentLocation, requestOrigin, node, mimeTypeGuess, extra)
  {
    ...
    if (socialWidgetHandler.isSocialWidgetTemporaryUnblock(contentLocation, window)) {
      return ACCEPT;
    }
...
let Policy = exports.Policy =
{
  ...
  isDisabledRequest: function(channel, window) {
    ...
    if (doDependingOnIsPrivate("disabledSites",
                               function(store) {
                                 return (topWin.location.host in store);
                               },
                               topWin)) {
      return true;
    }
  ...
  _isPreloadedWhitelistRequest: function(location, window) {
    ...
    if (status && cookieUtils.canAccessCookies(location, null)) {
      this._emitIfHostHasTracking(settingsMap, "update-settings", "noaction", window, host);
    }
    return status;
  },
  ...
  _isPolicyWhitelistRequest: function(location, window) {
    let host = location.host;
    if ((host in storage.policyWhitelist) && cookieUtils.canAccessCookies(location, null)) {
      this._emitIfHostHasTracking(settingsMap, "update-settings", "noaction", window, host);
      return true;
    }
    return false;
  ...
  isUserGreenRequest: function(location, window)
  {
    ...
    if (host in storage.userGreen) {
      if (window) {
        this._emitIfHostHasTracking(settingsMap, "update-settings", "usernoaction", window, host);
      }
      return true;
    }
    return false;
