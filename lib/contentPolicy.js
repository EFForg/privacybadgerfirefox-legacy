...
exports.ContentPolicy = Class({
  ...
  shouldLoad: function(contentType, contentLocation, requestOrigin, node, mimeTypeGuess, extra)
  {
    ...
    if (location.host in storage.userRed) {
      emit(settingsMap, "update-settings", "userblock", aWindow, location.host);
      return Ci.nsIContentPolicy.REJECT_REQUEST;
    }

    if (location.host in storage.blockedOrigins) {
      emit(settingsMap, "update-settings", "block", aWindow, location.host);
      return Ci.nsIContentPolicy.REJECT_REQUEST;
    }
