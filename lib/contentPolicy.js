...
let Policy = exports.Policy =
{
   ...
   *
   *  1. host or base domain is in userGreen
   *  2. host or base domain is in userYellow
   *  3. host or base domain is in the preloaded whitelist
   *
  ...
  heuristicCookieblocksRequest: function(location, window)
  {
    ...
    // Is the request's eTLD+1 on the heuristic blocklist AND host in the
    // preloaded whitelist?
  ...
  isUserRedRequest: function(location, window)
  {
    let origin = utils.getBaseDomain(location);
    let host = location.host;
    if ((origin in storage.userRed) || (host in storage.userRed)) {
  ...
  isUserYellowRequest: function(location, window)
  {
    let origin = utils.getBaseDomain(location);
    let host = location.host;
    if ((origin in storage.userYellow) || (host in storage.userYellow)) {
  ...
  isUserGreenRequest: function(location, window)
  {
    let origin = utils.getBaseDomain(location);
    let host = location.host;
    if ((origin in storage.userGreen) || (host in storage.userGreen)) {
