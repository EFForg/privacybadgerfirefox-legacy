...
let Policy = exports.Policy =
{
   ...
   * @return {Boolean}
   */
  _isPreloadedWhitelistRequest: function(location) {
    return utils.checkEachParentDomainString(location, function (host) {
      return (host in storage.preloads);
    });
  },
  ...
  heuristicBlocksRequest: function(location, window)
  {
    ...
    if (self.isUserGreenRequest(location, window) ||
        ...
        self._isPreloadedWhitelistRequest(location) ||
  ...
  heuristicCookieblocksRequest: function(location, window)
  {
    ...
    if (self._isPreloadedWhitelistRequest(location)) {
      if ((origin in storage.blockedOrigins) &&
          !(host in storage.policyWhitelist)) {
