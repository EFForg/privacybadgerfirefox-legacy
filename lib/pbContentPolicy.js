...
exports.pbContentPolicy = Class({
  ...
  shouldLoad: function(contentType, contentLocation, requestOrigin, node, mimeTypeGuess, extra)
  {
    ...
    let channel = location.QueryInterface(Ci.nsIHttpChannel);
    if (Policy.shouldIgnoreRequest(channel))
    {
      return Ci.nsIContentPolicy.ACCEPT;
    }

    // This is where blocking happens for userRed sites
    if (Policy.shouldBlockRequest(channel))
    {
...
let Policy = exports.Policy =
{
   ...
   * @return {Boolean}
   */
  shouldIgnoreRequest: function(channel)
  {
    if (!utils.isThirdPartyChannel(channel)) { return false; }
    return (!Policy.isBlockableScheme(channel.URI)
            || Policy.isWhitelisted(channel.URI.spec));
  },

  /**
   * Checks whether the site has been blocked by the heuristic blocker.
   * @param location  {nsIChannel}
   * @return {Boolean}
   */
  shouldBlockRequest: function(channel)
  {
    return (channel.URI.hostname in storage["userRed"]);
  }
};
